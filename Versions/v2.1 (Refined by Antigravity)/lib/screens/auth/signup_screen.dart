...
class SignupScreen extends ConsumerWidget {
  ...
  Future<void> _handleSignup(
    ...
    try {
      final credential = await ref.read(firebaseServiceProvider).signUp(
            emailController.text.trim(),
            passwordController.text.trim(),
          );
      final user = credential.user;
      if (user != null) {
        final name = ref.read(_signupNameControllerProvider).text.trim();
        await FirebaseFirestore.instance.collection('users').doc(user.uid).set({
          'name': name.isNotEmpty ? name : 'Jane Doe',
          'email': user.email ?? emailController.text.trim(),
          'mealsSaved': 0,
          'totalWeightSaved': 0.0,
          'aiMessage': 'Welcome to FoodRescue! Save surplus food to offset environmental footprint.',
        });
      }
